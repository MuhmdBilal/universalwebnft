...
import {OpenImage,FormHandle, GetNews,AddNews,UpdateList} from '../../../scripts/news/news';
import {useMutation} from '@apollo/client';
import { withApollo } from '@apollo/client/react/hoc';
import { ADD_EDIT_NEWS } from '../../../queries/news';
...
const AddEditNews = ({SetShow,newsID,client,listVariables}) => {

    const [mainPhoto, SetMainPhoto] = useState("");
    const [formData,SetFormData] = useState(INIT_FORM_DATA);

    const [AddEditNews,{loading,error}] = useMutation(ADD_EDIT_NEWS,{
        onCompleted:(data) => {
        ...
        update:(cache,response) => {
            UpdateList(cache,response,listVariables,newsID);
    ...
    useEffect(() => {
        ...
        if(newsID != 0)
        {
            GetNews(client,newsID,SetFormData,SetMainPhoto);
        }
        ...
        >
            <div id="admin-modal-content">
                ...
                <div className="header">
                    <h3>{newsID != 0 ? "Upravit novinku" : "Přidat novinku"}</h3>
                ...
                {!loading && !err ?
                    <div className="footer">
                        <button type="button" className="btn btn-primary smaller" onClick={() => AddNews(AddEditNews,formData,newsID,client)}>{(newsID && newsID != 0 ? "Uložit" : "Přidat")}</button>
...
export default withApollo(AddEditNews);